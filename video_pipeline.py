...
def get_openai_client():
    ...
    return OpenAI(
        base_url="https://haithamismail-hi--qwen-2-5-32b-vllm-serve.modal.run/v1",
        api_key=os.environ["MODAL_API_KEY"]
    )

def get_hf_client():
    if not config.HF_API_KEY or "default_key" in config.HF_API_KEY:
        raise ValueError("HF_API_KEY environment variable not set or is a placeholder.")
    return InferenceClient(model=config.CLIP_MODEL_NAME, api_key=config.HF_API_KEY)
...
def compare_frames_threaded_batched(video_path, batch_size=32, queue_size=2):
    ...
    while True:
        ...
        batch_features = get_image_embedding(batch_images, CLIP_EMBEDDING_URL)
...
def run_pipeline(video_path):
    ...
    embeddings = get_text_embedding(texts_to_embed, CLIP_EMBEDDING_URL)
